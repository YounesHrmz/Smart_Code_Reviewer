...
class ComplexityAnalyzer:
    ...
    def analyze_snippet(cls, code: str) -> dict:
        ...
        if complexity > 5:
            issues.append(
                f"المشكلة: تعقيد دوري مرتفع (الدرجة: {complexity}) مما يجعل هذا الجزء صعب الاختبار والصيانة. التوصيات الهندسية: 1) قسم المنطق إلى دوال أصغر. 2) أخرج الشروط المعقدة إلى وظائف منفصلة. 3) أضف اختبارات وحدية لكل فرع من فروع المنطق."
            )
            severity = "warning"
            if complexity > 10:
                severity = "critical"
        ...
        if code.count("    ") >= 4 or code.count("\t") >= 4:
            issues.append(
                "المشكلة: تم اكتشاف تداخل عميق في البنية. التوصيات الهندسية: 1) قسم هذا المنطق إلى دوال أو وحدات أصغر. 2) قلّل مستوى التداخل باستخدام استرجاع منطق منفصل. 3) حسّن البنية لتسهيل القراءة والتوسع المستقبلي."
            )
