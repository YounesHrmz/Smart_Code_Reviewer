...
class ASTParser:
    ...
    def get_isolated_snippets(self) -> list:
        ...
        if not self.tree:
            return [{'line_no': i + 1, 'code': line.strip()} for i, line in enumerate(lines) if line.strip()]

        for node in ast.walk(self.tree):
            if isinstance(node, (ast.FunctionDef, ast.Assign, ast.Call, ast.Expr)):
                start = node.lineno
                end = getattr(node, 'end_lineno', start)
                if end and end <= len(lines):
                    snippet_text = "\n".join(lines[start-1:end]).strip()
                    if snippet_text and not any(s['code'] == snippet_text for s in snippets):
    ...
    def extract_structural_metrics(self) -> dict:
        ...
        metrics = {
            ...
            'num_loops': 0, 'num_conditions': 0, 'num_comments': 0,
            'num_docstrings': 0, 'max_nesting_depth': 0, 'avg_function_length': 0
        }
        
        for node in ast.walk(self.tree):
            if isinstance(node, ast.FunctionDef): metrics['num_functions'] += 1
            ...
            elif isinstance(node, (ast.For, ast.While)): metrics['num_loops'] += 1
            elif isinstance(node, ast.If): metrics['num_conditions'] += 1
